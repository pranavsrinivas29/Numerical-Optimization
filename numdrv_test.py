...
def test(fun):
    dict = {"drv": test_drv, "ddrv": test_ddrv, "pdrv": test_pdrv, "grad": test_grad, "jac": test_jac, "hess": test_hess}
...
def my_plot(f, N=100, x_min=0., x_max=1., y_min=0., y_max=1.):
    ...
    plt.title(r"max$ |f| = $" + "{:1.2e}".format( maxval))
...
def test_jac(jac):
    ...
    # Analytical d^2/dx^2 + d^2/dy^2
    def g_analytical_jacobean(z):
        x,y = z
        mat = np.eye(2)
        mat[0,0] = 2*(x - 0.5)
        mat[1,1] = 2*np.pi*np.cos(2*np.pi*y)
        return mat

    def error(x):
        num_jacobi = jac(g, x, 1.e-4)
        return np.linalg.norm(g_analytical_jacobean(x) - num_jacobi )
